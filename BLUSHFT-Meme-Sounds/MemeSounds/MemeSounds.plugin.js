 ...
 * @source https://github.com/BlueshiftTheProtogen/BLUSHFT-MemeSounds/tree/master/BLUSHFT-Meme-Sounds/MemeSounds/MemeSounds.plugin.js
 * @updateUrl https://raw.githubusercontent.com/BlueshiftTheProtogen/BLUSHFT-MemeSounds/tree/master/BLUSHFT-Meme-Sounds/MemeSounds/MemeSounds.plugin.js
 */
	...
	return !global.ZeresPluginLibrary ? class {
		...
		const plugin = (Plugin, Api) => { try {
			...
			const sounds = [
				...
				{re: /C4/gmi, file: "c4.mp3", duration: 100},
				{re: /CASSIE/gmi, file: "cassie.mp3", duration: 15000},
				{re: /mtf/gmi, file: "mtf.mp3", duration: 900}
			];
				...
				messageEvent = async ({ channelId, message, optimistic }) => {
					...
					if (!optimistic && lastMessageID != message.id) {
						...
						for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
							let audio = new Audio("https://github.com/BlueshiftTheProtogen/BLUSHFT-MemeSounds/tree/master/BLUSHFT-Meme-Sounds/MemeSounds/Sounds/"+sound[1].file);
